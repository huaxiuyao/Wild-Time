...
from data.weather.preprocess import preprocess
...
class WeatherBase(Dataset):
    def __init__(self, args):
        ...
        for i in self.ENV:
            # Regularize temperatures for regression
            # print(i)
            # year_temps = self.datasets[i][self.mode]['temperatures'].reshape(-1, 1)
            # year_temps_norm = self.scaler.transform(year_temps)
            # year_temps = self.datasets[i][self.mode]['temperatures']
            # year_temps_norm = (year_temps - min_temp) / (max_temp - min_temp)
            # self.datasets[i][self.mode]['temperatures'] = year_temps_norm
            # print(np.sum(year_temps == rev_min_max_func(year_temps_norm, self.all_temps)))
            # print('len', len(year_temps))
        ...
        #     num_samples = 0
        #     for i in self.ENV:
        #         sel_idx = np.nonzero(self.datasets[i][Mode.TEST_OOD]['labels'] == classid)[0]
        #         num_samples += len(sel_idx)
        #     print(classid, num_samples)
...
class WeatherGroup(WeatherBase):
    def __init__(self, args):
        super().__init__(args=args)
