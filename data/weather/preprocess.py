...
RAW_DATA_FILE = 'full-dataset/shifts_weather.csv'
ID_HELD_OUT = 0.1
...
class Partitioner():
    ...
    def save(self, save_path):
        ...
        for taskid in self.dfs_to_save.keys():
            ...
            weather_dataset[taskid] = {}
            weather_dataset[taskid][Mode.TRAIN] = {}
            weather_dataset[taskid][Mode.TRAIN]['data'] = {}
            weather_dataset[taskid][Mode.TRAIN]['data']['categorical'] = np.array(categorical_data.to_numpy())[
                train_idxs]
            weather_dataset[taskid][Mode.TRAIN]['data']['continuous'] = np.array(continuous_data.to_numpy())[train_idxs]
            weather_dataset[taskid][Mode.TRAIN]['labels'] = np.array(precipitation_labels.to_numpy())[train_idxs]
            weather_dataset[taskid][Mode.TRAIN]['temperatures'] = np.array(temperatures.to_numpy())[train_idxs]
            weather_dataset[taskid][Mode.TEST_ID] = {}
            weather_dataset[taskid][Mode.TEST_ID]['data'] = {}
            weather_dataset[taskid][Mode.TEST_ID]['data']['categorical'] = np.array(categorical_data.to_numpy())[
                test_idxs]
            weather_dataset[taskid][Mode.TEST_ID]['data']['continuous'] = np.array(continuous_data.to_numpy())[
                test_idxs]
            weather_dataset[taskid][Mode.TEST_ID]['labels'] = np.array(precipitation_labels.to_numpy())[test_idxs]
            weather_dataset[taskid][Mode.TEST_ID]['temperatures'] = np.array(temperatures.to_numpy())[test_idxs]
            weather_dataset[taskid][Mode.TEST_OOD] = {}
            weather_dataset[taskid][Mode.TEST_OOD]['data'] = {}
            weather_dataset[taskid][Mode.TEST_OOD]['data']['categorical'] = np.array(categorical_data.to_numpy())
            weather_dataset[taskid][Mode.TEST_OOD]['data']['continuous'] = np.array(continuous_data.to_numpy())
            weather_dataset[taskid][Mode.TEST_OOD]['labels'] = precipitation_labels.to_numpy()
            weather_dataset[taskid][Mode.TEST_OOD]['temperatures'] = temperatures.to_numpy()

        with open(save_path, 'wb') as f:
            pickle.dump(weather_dataset, f)
...
def preprocess(args):
    ...
    print()
    # Save all files
    preprocessed_data_path = os.path.join(args.data_dir, 'weather.pkl')
