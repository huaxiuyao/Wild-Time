...
def _augmix_aug(x: torch.Tensor):
    ...
    x_img = tensor_to_PIL(x)
    for i in range(3):
        x_aug = x_img.copy()
        for _ in range(np.random.randint(1, 4)):
            x_aug = np.random.choice(augmentations)(x_aug)
        # Yearbook dataset images were downsampled to 32, transforms upsize to 224, need to downsize again
        x_aug_tensor = PIL_to_tensor(x_aug)
        if not x_aug_tensor.shape[1] == x.shape[1]:
            x_aug_tensor = PIL_to_tensor(x_aug.resize((x.shape[1], x.shape[2])))
        mix += w[i] * x_aug_tensor.cuda()
    mix = m * x + (1 - m) * mix
    return mix
