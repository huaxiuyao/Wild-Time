...
class LossComputer:
    def __init__(self, dataset, criterion, is_robust, alpha=0.2, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01,
        ...
        if adj is not None:
            ...
            self.adj = torch.zeros(self.num_groups).float().cuda()

        if is_robust:
            assert alpha, 'alpha must be specified'

        # quantities maintained throughout training
        self.adv_probs = torch.ones(self.num_groups).cuda() / self.num_groups
        self.exp_avg_loss = torch.zeros(self.num_groups).cuda()
        self.exp_avg_initialized = torch.zeros(self.num_groups).byte().cuda()

        self.reset_stats()

    def loss(self, yhat, y, group_idx=None, is_training=False):
        # compute per-sample and per-group losses
        if str(self.dataset) == 'drug':
            per_sample_losses = self.criterion(yhat, y.unsqueeze(1))
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        # group_idx is of shape (bs,) containing the group index of each example in the batc
        group_map = (group_idx == torch.arange(self.num_groups).unsqueeze(1).long().cuda()).float()  # (num_groups, bs)
        group_count = group_map.sum(1)  # (4)
        group_denom = group_count + (group_count == 0).float()  # avoid nans, (num_groups)
    ...
    def reset_stats(self):
        self.processed_data_counts = torch.zeros(self.num_groups).cuda()
        self.update_data_counts = torch.zeros(self.num_groups).cuda()
        self.update_batch_counts = torch.zeros(self.num_groups).cuda()
        self.avg_group_loss = torch.zeros(self.num_groups).cuda()
        self.avg_group_acc = torch.zeros(self.num_groups).cuda()
